...
result in larger values at particular dimensions.  The parameter
B<vect_sign> can be used to enable a signed embedding.  That is, one bit of
the hash function is used to define a sign for each feature.  Collisions are
still possible, yet their impact is lessened as colliding features not
necessary induce larger values.
...
dimensions is non-zero, 20 to 24 bits are often sufficient for representing
