...
can be escaped using URI encoding (%XX).  If no delimiters are specified
using B<ngram_delim>, all occurences of the stop words are removed.  If
delimiters are given, only occurrences between delimiters are filtered.
