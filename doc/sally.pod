...
of string data. B<sally> can applied to several types of string data,
...
output format, are specified in the file I<config> (see
L</CONFIGURATION>)
...
such as plain text or in LibSVM or Matlab format.

The processing chain of B<sally> makes heavy use of sparse data
structures, such that high-dimensional vectors with millions of
dimensions can be handled efficiently.  Moreover, all features and
dimensions are indexed by an efficient hash function, where the bit
width of this function can be adjusted according to the
characteristics of the data.  In several settings, a hash width
between 24 and 28 bit already suffices to store millions of features
...
space.  Following is a list of supported feature sets:
...
LibSVM.  Following is a list of output formats supported by B<sally>:
...
The feature vectors of the embedded strings are stored as a plain text
using a list of dimensions, features and respective values.  Almost
all comprehensive analysis tools can deal with this type of format.
...
given as I<output> to B<sally>.

=back

=head1 CONFIGURATION

The configuration of B<sally> is specified in the file I<config>. It
is structured into the three sections B<input>, B<features> and
B<output>, which define the parameters of the input format, the
...
This parameter specifies the input format. Supported values are "dir",
"arc", "lines" and "fasta".  For the "dir" and "arc" format, the input
strings are provided as either a directory or an archive of files. For
the "lines" format the strings are available as lines in a text file.
Similarly, the "fasta" format expects a text file in FASTA format.
...
This parameter specifies the length of n-grams.  It determines the
numbers of consecutive words that are considered as one feature.  If
set to 1, the embedding of B<sally> matches a classical bag-of-words
model, where each word is associated with one dimension of the feature
space. For larger values, short sequences of consecutive words are
associated with the dimensions of the feature space, resulting in a
more complex embedding.

=item B<ngram_delim = "%0a%0d";>

This parameter defines characters for delimiting words in strings.
The characters can be either specified as regular bytes or as
hexadecimal numbers prefixed by "%" (similar to URI encoding). If no
characters are specified, the strings are analyzed at byte-level, that
is, each byte reflects one word. This settings is useful if the
...
in the vector space. This parameter specifies the number of bits used
by this hash function and defines the maximum dimensionality of the
vector space with 2^bits. as the embedding of B<sally> is usually
sparse and only a small fraction of dimensions is non-zero, 24 to 28
bits are often sufficient for representing the data.  If this
parameter is chosen too small, the embedding may suffer from
collisions of features in the vector space.
...
weights will be read from the file. Keeping a separate file fo TF-IDF
...
This parameter specifies the output format. Supported values are
"libsvm", "text" and "matlab". For "libsvm", B<sally> generates data
files suitable for processing using the tools LibSVM and SVMLight. The
"text" and "matlab" formats target a generic analysis, where the later
format can be read from a Matlab and Octave environment.
