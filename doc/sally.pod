...
The processing chain of B<sally> makes heavy use of sparse data structures,
such that high-dimensional vectors with millions of dimensions can be
handled efficiently.  Moreover, all features and dimensions are indexed by
an efficient hash function, where the bit width of this function can be
adjusted according to the characteristics of the data.  In several settings,
a hash width between 24 and 28 bit already suffices to store millions of
features with little loss of accuracy.
...
The strings loaded by B<sally> are characterized by a set of features, where
each feature is associated with one dimension of the vector space. 
Following is a list of supported feature sets.  More details on defining and
tuning these features are provided in the configuration section of this
document.
...
Once the input strings have been embedded in a vector space, B<sally> stores
the resulting vectors in one of several common formats, which allows for
applying typical tools of statistics and machine learning to the data, for
example, Matlab, Octave, Shogun, Weka, SVMLight and LibSVM.  Following is a
list of output formats supported by B<sally>.

=over 14

=item B<libsvm> 

The feature vectors of the embedded strings are stored in the common libsvm
format, which is supported by Shogun, SVMLight and LibSVM. The name of the
output file is given as I<output> to B<sally>.

=item B<text> 

The feature vectors of the embedded strings are stored as a plain text using
a list of dimensions, features and respective values.  Almost all
compherensive analysis tools can deal with this type of format.  The name of
the output file is given as I<output> to B<sally>.
...
The configuration of B<sally> is specified in the file I<config>. The
configuration is structured into three sections B<input>, B<features> and
B<output>, which define the parameters of the input format, the feature
extraction and the output format, respectively.
...
strings are provided as either a directory or an archive of files.  For the
"lines" format the strings are availables as lines in a text file. 
Similarly, the "fasta" format expects a text file in FASTA format.

=item B<chunk_size = 256;>

This parameter defines the number of strings to process in each cunk. To
enable an efficient processing of large data sets, B<sally> processes
strings in chunks.  Depending on the length of the strings, this parameter
can be adjusted to balance loading and processing of data.

=item B<fasta_regex = " (\\+|-)[:digit:]*";>
...
useful for application of supervised learning methods.  The parameter is a
regular expression matching labels, such as +1 and -1.
