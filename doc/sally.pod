...
string features associated with each dimension are hash and sorted multiple
times as proposed by Broder (1997).  In each round the smallest hash value
is appended to the minimum hash.  The number of hash bits in each round is
defined by B<hash_bits> in the feature configuration section and the number
of rounds is simply determined by ceil(B<dim_num> / B<hash_bits>).
